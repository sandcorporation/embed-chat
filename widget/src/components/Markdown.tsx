...
const heading = (em: string): CSSProperties => ({ margin: '8px 0 4px', fontWeight: 600, fontSize: em, lineHeight: 1.3 })
...
const components: Components = {
  ...
  h1: ({ children }) => <h1 style={heading('1.2em')}>{children}</h1>,
  h2: ({ children }) => <h2 style={heading('1.1em')}>{children}</h2>,
  h3: ({ children }) => <h3 style={heading('1.05em')}>{children}</h3>,
  h4: ({ children }) => <h4 style={heading('1em')}>{children}</h4>,
  h5: ({ children }) => <h5 style={heading('1em')}>{children}</h5>,
  h6: ({ children }) => <h6 style={heading('1em')}>{children}</h6>,
 ...
 * - raw HTML 미파싱(react-markdown 기본), 이미지 비활성, 링크는 새 탭·rel 보안·프로토콜 화이트리스트
