...
def test_untrusted_content_delimited_and_anti_disclosure_present():
    ...
    # 비신뢰 데이터 구역 + delimiter
    assert "신뢰할 수 없는 데이터" in system
    ...
    assert system.index("IGNORE PREVIOUS INSTRUCTIONS") > system.index("UNTRUSTED_DATA")
    # 원래 Tenant system 프롬프트는 앞에 유지
